...
**This softare is under development and an installer has not yet been publicly released.
Initial code has been copied from the AWS plugin and is being updated.**

This repository contains the Open Water Foundation TSTool Google Drive plugin.
This plugin can be installed to enable commands that integrate TSTool with Google Drive,
for example to access files on Google G: stream drive from Linux.

TSTool is part of [Colorado's Decision Support Systems (CDSS)](https://www.colorado.gov/cdss).
See the following online resources:

*   [TSTool Google Plugin download page](https://software.openwaterfoundation.org/tstool-googledrive-plugin/)
*   [TSTool Google Plugin Documentation](https://software.openwaterfoundation.org/tstool-googledrive-plugin/latest/doc-user/)
  ...
  cdss-dev/                              Recommended folder for CDSS product development.
    TSTool/                              Recommended folder for TSTool development.
      git-repos/                         Recommended folder for TSTool product repositories.
        owf-tstool-googledrive-plugin/   Repository working files.
        ...
        owf-tstool-googledrive-plugin/   Maven project source code and supporting files.
...
This project is configured using Maven, which manages third-party dependencies, such as Google packages.
...
This section explains how to add the TSTool Google plugin to the TSTool core development environment.
...
6.  Browse to the `cdss-dev/TSTool/git-repos/owf-tstool-googledrive-plugin/owf-tstool-googledrive-plugin` folder
    and ***Select Folder***.
...
9.  Right click on `owf-tstool-googledrive-plugin` in ***Package Explorer*** or ***Project Explorer***
...
1.  Eclipse project:
    1.  The `owf-tstool-googledrive-plugin` repository is added as a Maven project.
    2.  The project's build path is configured to use appropriate Maven dependencies,
        such as the Google libraries and other TSTool projects, such as `cdss-lib-common-java`.
2.  Plugin jar file:
    ...
    3.  The above script relies on the `owf-tstool-googledrive-plugin/src/main/resources/META-INF/MANIFEST.MF`
...
1.  Update the version:
    1.  Update the `src/main/java/owf-tstool-googledrive-plugin/org/openwaterfoundation/tstool/plugin/google/PluginMeta.java` source file.
