#!/bin/bash
#
# Create the TSTool Google Drive plugin product 'index.html' page on Amazon S3,
...
#
# software.openwaterfoundation.org/tstool-googledrive-plugin/index.html
# software.openwaterfoundation.org/tstool-googledrive-plugin/index.csv
#
...
#   2018-12-04 16:16:22          0 tstool-googledrive-plugin/
#   2018-12-04 16:16:37          0 tstool-googledrive-plugin/1.0.0/
#   2018-12-04 16:17:19   46281975 tstool-googledrive-plugin/1.0.0/tstool-googledrive-plugin-win-1.0.0-2201020304.zip
createCatalogFile() {
  logInfo "Creating catalog file from contents of Amazon S3 files."
  # For debugging.
  #set -x
  logInfo "Listing Google Drive S3 installers with:"
  ...
  if [ ${errorCode} -ne 0 ]; then
    logError ""
    logError "Error listing TSTool Google Drive plugin download files to create catalog (maybe missing --aws-profile)?"
    exit 1
  fi
  ...
  tmpS3CatalogPath="/tmp/${USER}-tstool-googledrive-plugin-catalog-ls-installers.txt"
  # Only list installers that match the current download filename specification
  cat ${tmpS3ListingPath} | grep -E 'tstool-googledrive-plugin.*.zip' > ${tmpS3CatalogPath}
  logInfo "Available TSTool Google Drive plugin installers:"
...
createIndexHtmlFiles() {
  ...
  indexHtmlTmpFile="/tmp/${USER}-tstool-googledrive-plugin-index.html"
...
<title>TSTool Google Drive Plugin Downloads</title>
</head>' >> ${indexHtmlTmpFile}

echo '
<body>
<h1>Open Water Foundation TSTool Google Drive Plugin Software Downloads</h1>
<p>
The TSTool Google Drive Plugin software is available for Windows.
...
<li> See the latest <a href="https://software.openwaterfoundation.org/tstool-googledrive-plugin/latest/doc-user/appendix-install/install/">TSTool Google Drive Plugin installation documentation</a>
     for installation information (or follow a link below for specific version documentation).</li>
<li>The TSTool Google Drive Plugin requires that TSTool is also installed if not already installed:
    <ul>
...
<ol>
<li> Download a TSTool Google Drive Plugin Windows installer (zip file) from below.</li>
<li> Follow the latest <a href="https://software.openwaterfoundation.org/tstool-googledrive-plugin/latest/doc-user/appendix-install/install/">installation instructions</a>
...
<p>Linux versions of the TSTool Google Drive Plugin are not actively developed.
    ...
    cat "${tmpS3CatalogPath}" | grep "${downloadOs}-" | awk '{ printf "%s %s %s %s\n", $1, $2, $3, $4 }' | sed -E 's|([0-9][0-9]/)|\1-zzz|g' | sed 's|/-zzz|-zzz|g' | sed 's|dev|-dev|g' | sort -r -k4,4 | sed 's|-zzz||g' | sed 's|-dev|dev|g'  | awk '
      {
        ...
        downloadFileProduct=""            # The product name (e.g., TSTool Google Drive Plugin)
        ...
        if ( filenameSpecVersion = 1 ) {
          # Initial filename spec using format:  tstool-googledrive-plugin/1.0.0.dev/software/tstool-googledrive-plugin-1.0.0.dev-win-2201020304.zip
          ...
          downloadFileProduct="tstool-googledrive-plugin"
          downloadFileVersion=downloadFileParts[4]
          downloadFileOs=downloadFileParts[5]
        }
        # Set the URL to the download file.
        downloadFileUrl=sprintf("https://software.openwaterfoundation.org/tstool-googledrive-plugin/%s/software/%s", downloadFileVersion, downloadFile)
        product = "TSTool Google Drive Plugin"
        ...
        docDevUrl=sprintf("https://software.openwaterfoundation.org/tstool-googledrive-plugin/%s/doc-dev/",downloadFileVersion)
        ...
        docUserUrl=sprintf("https://software.openwaterfoundation.org/tstool-googledrive-plugin/%s/doc-user/",downloadFileVersion)
...
printUsage() {
  ...
  echoStderr ""
  echoStderr "Create the software.openwaterfoundation.org/tstool-googledrive-plugin/index.html and related files."
...
printVersion() {
  ...
  echoStderr ""
  echoStderr "TSTool Google Drive Plugin"
...
uploadIndexFiles() {
  ...
  logInfo "Assuming that TSTool Google Drive Plugin installer files were previously uploaded to S3."
  ...
  if [ -f "${s3CatalogTxtFileUrl}" ]; then
    ...
    if [ ${errorCode} -ne 0 ]; then
      logError ""
      logError "Error uploading TSTool Google Drive Plugin catalog file."
      # Not fatal.
    fi
  else
  ...
  #                "Quantity": 1,
  #                "Items": [
  #                    "/tstool-googledrive-plugin/index*"
  #                ]
  #            },
  ...
  #${awsExe} cloudfront create-invalidation --distribution-id ${cloudFrontDistributionId} --paths "/tstool-googledrive-plugin/index.html*" "/tstool-googledrive-plugin/" "/tstool-googledrive-plugin" --profile "${awsProfile}"
  #${awsExe} cloudfront create-invalidation --distribution-id ${cloudFrontDistributionId} --paths "/tstool-googledrive-plugin/index.html*" --profile "${awsProfile}"
  if [ "${operatingSystem}" = "mingw" ]; then
    ...
    MSYS_NO_PATHCONV=1 ${awsExe} cloudfront create-invalidation --distribution-id "${cloudFrontDistributionId}" --paths '/tstool-googledrive-plugin/index.html' '/tstool-googledrive-plugin' '/tstool-googledrive-plugin/' "/tstool-googledrive-plugin/${indexFaviconFile}" --output json --profile "${awsProfile}" | tee ${tmpFile}
  else
    ${awsExe} cloudfront create-invalidation --distribution-id "${cloudFrontDistributionId}" --paths '/tstool-googledrive-plugin/index*' '/tstool-googledrive-plugin' '/tstool-googledrive-plugin/' "/tstool-googledrive-plugin/${indexFaviconFile}" --output json --profile "${awsProfile}" | tee ${tmpFile}
  fi
  #${awsExe} cloudfront create-invalidation --distribution-id ${cloudFrontDistributionId} --paths "/tstool-googledrive-plugin" --profile "${awsProfile}"
...
user=""
tmpS3ListingPath="/tmp/${USER}-tstool-googledrive-plugin-catalog-ls.txt"
