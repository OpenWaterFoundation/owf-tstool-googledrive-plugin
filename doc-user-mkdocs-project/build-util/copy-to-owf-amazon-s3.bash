...
dryrun=""
#dryrun="--dryrun"
s3VersionFolder="s3://software.openwaterfoundation.org/tstool-googledrive-plugin/${pluginVersion}/doc-user"
s3LatestFolder="s3://software.openwaterfoundation.org/tstool-googledrive-plugin/latest/doc-user"
...
if [ -n "${pluginVersion}" ]; then
  ...
  #logInfo "Running: ${awsExe} cloudfront create-invalidation --distribution-id ${cloudFrontDistributionId} --paths \"/tstool-googledrive-plugin/${pluginVersion}/doc-user/*\" --output json --profile \"${awsProfile}\""
  if [ "${operatingSystem}" = "mingw" ]; then
    ...
    MSYS_NO_PATHCONV=1 ${awsExe} cloudfront create-invalidation --distribution-id ${cloudFrontDistributionId} --paths "/tstool-googledrive-plugin/${pluginVersion}/doc-user/*" --output json --profile "${awsProfile}" | tee ${tmpFile}
  else
    ${awsExe} cloudfront create-invalidation --distribution-id ${cloudFrontDistributionId} --paths "/tstool-googledrive-plugin/${pluginVersion}/doc-user/*" --output json --profile "${awsProfile}" | tee ${tmpFile}
  fi
...
if [ "${answer}" = "y" ]; then 
  ...
  logInfo "Invalidating files so that CloudFront will make new files available..."
  logInfo "Running: ${awsExe} cloudfront create-invalidation --distribution-id ${cloudFrontDistributionId} --paths \"/tstool-googledrive-plugin/latest/doc-user/*\" --output json --profile \"${awsProfile}\""
  ${awsExe} cloudfront create-invalidation --distribution-id ${cloudFrontDistributionId} --paths "/tstool-googledrive-plugin/latest/doc-user/*" --output json --profile "${awsProfile}" | tee ${tmpFile}
  errorCode=${PIPESTATUS[0]}
  if [ ${errorCode} -ne 0 ]; then
    logError " "
    logError "Error invalidating CloudFront file(s)."
